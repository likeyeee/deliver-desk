...
登录后先在智联网站确认在线简历。工作台会逐个读取符合条件的职位详情，核对岗位、公司和职位 ID，再点击“立即投递”。网站同时发送在线简历和平台招呼，出现成功弹窗后记录结果与实际招呼，再处理下一岗。无需设置 AI 密钥或上传本地简历，不额外追加自定义消息。

“更多匹配条件”里的公司行业与职位类型需按网页层级填写，例如“汽车/摩托车/电动车 > 汽车4S店/经销商”；要选择整个大类，可在末级填写“不限”。其他条件使用智联网页显示的选项文字。

已投递、已沟通和待核对的职位会跳过；回执不明确时停止且不会自动重投。“只读核对送达”会检查智联的求职反馈，要求同一职位 ID、岗位、公司与“成功投递”状态一致。“继续沟通”按钮本身不足以确认简历已送达。消息回复和自动回复目前支持 BOSS，智联后续聊天请在网页处理。
