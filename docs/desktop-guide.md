...
省市列表覆盖 34 个省级地区、373 个网站城市选项；切换省份后会选中该省的第一个城市，可继续选择目标城市。选“全国”恢复全国范围。已保存的城市会自动匹配省份，旧配置中手动填写的其他城市也会保留。切换城市会清除旧城市的“工作区域”，其他条件继续保留。[城市目录来源](research/regions.md)
...
在“个人简历”上传 PDF、DOCX、TXT 或粘贴正文，点击“分析简历”，核对 DeepSeek 提取的个人概况、技能、经历和优势后保存。点击“使用 AI 岗位招呼”回到工作台，可设置招呼要求、选择已发现的职位生成预览。正式投递会按各岗位最新详情结合简历分别生成；生成失败时停止，不发起沟通。详细流程与文件要求见 [简历与岗位招呼](resume-greetings.md)。
...
- 在“偏好与数据 → 打开目录”可找到本机数据。配置导出带任务设置、模型参数和系统提示词，不含 API Key、简历正文或分析结果；提示词和 CSV 可能带个人资料，请按需要分享。
- 导入简历只在本机提取文字，点击分析或使用 AI 岗位招呼后，简历正文、特点和相关职位资料会发送至 DeepSeek。简历不会自动作为附件发送给招聘方。移除简历不删除原文件和已有投递记录。
