...
4. 预览只保存发现记录。AI 模式使用本次冻结的简历、特点与配置，结合当前职位详情生成完整招呼；失败或取消时不进入沟通。发送前再次核对职位，在事务中检查历史并登记 `sending`，占用尝试名额。
...
简历导入由主进程的文件选择器提供路径；Python 在本机提取 PDF、DOCX 或 TXT 正文，只把正文和分析结果写入独立的 `resume.json`，不复制原文件。分析、预览和投递共用任务锁；简历修订号用于拒绝过期分析与预览，修改正文会清除旧特点。配置导出不包含简历，发布审计也排除该私有文件。招呼预览仅保存在内存，实际发送的完整正文仍写入 `deliveries`，遵循现有的去重和回执逻辑。
