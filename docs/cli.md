...
BOSS 的“投递记录”保存招呼和沟通结果，智联的记录保存在线简历投递结果。程序使用智联账户中已有的在线简历，不上传本机简历附件，也不把“已发出”当作“已读”。随机延时用于控制操作节奏；程序不处理验证码、不使用隐身补丁、不轮换账号或代理。
...
职位详情 URL 的职位 ID 是唯一键，智联加 `zhaopin:` 平台前缀，追踪参数和 `securityId` 不进入历史。每次真实点击前，在 SQLite 事务中检查重复、占用当日名额并登记 `sending`。数据库启用 WAL，独占锁限制同一数据目录只运行一个浏览器任务；两个平台共用该目录的每日尝试额度。
